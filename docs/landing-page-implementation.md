...
Trust is split across two sections on the `examples-of-trust` branch. "Trust builds over time" keeps the SVG curve for a single generic project with three signals that apply to any field: project update, peer endorsement, and funding record. Select a signal to view that stage of the curve. "What counts as evidence depends on the field" draws a project's public record as a countersigned document: the three common lines stay fixed and only the field-specific lines change, with a seal mark and issuer in accent. The fields cycle automatically every 4.5 seconds while visible and stop once the reader hovers, focuses, or selects a field; reduced motion disables the cycle. Field tabs support arrow keys, Home, and End. The descriptions and issuers are illustrative examples, not real records, and nothing is a numerical or universal trust score.

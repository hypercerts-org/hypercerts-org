...
The homepage follows the latest Miro continuity pass: hero, mission, moving partners, challenge, information-reuse bridge, selectable trust example, protocol roles, Ma Earth, why now, information-economy timeline, collective, media, contact, and footer. The header now links Home, About, and Docs. Existing media remain. The Ma Earth case study is the blog post at `/blog/3mvru46xfus2r`; the separate `/case-studies/ma-earth` page was removed on 18 September while there is only one use case, and the landing Ma Earth section links to the post. The hero buttons are "See why it matters" (primary, scrolling to the challenge) and "Start a conversation" (secondary, to the contact form).
...
## Why now, moved up (18 September)

"Why now" now follows the challenge instead of the Ma Earth section, as in the V2 overview document ("Hypercerts: Open Infrastructure for Funding Valuable Work V2", Linear): the challenge, then the two shifts that make it urgent, then the opportunity. The order is hero, partners, challenge, why now, the opportunity bridge, trust example, protocol, Ma Earth, funding models, builders, mission, collective, media, and contact. `KnowledgeBridge` is now placed in `app/page.tsx` instead of inside `ResourceAllocation`, so "Why now" can sit between them. Backgrounds still alternate: grey challenge, cream why now, white bridge, cream trust; the funding-models section is now white, so it does not sit cream against cream beside Ma Earth.

The copy follows the document's two shifts. The heading is "Information is cheap to produce, and expensive to verify", balanced so no word sits alone on narrow screens. The left column says a polished application no longer shows whether the project behind it does good work; the right column says AI is moving into funding decisions, and whoever holds the data sets the terms. The earlier right column, on AT Protocol records for people and agents, was a solution statement and is covered by the protocol section. The section closes with a centered statement in a white card, set like a quote in grey serif italic with one sentence per line, rather than styled like another heading: "The infrastructure for AI-guided funding is being built today. It must be open, not owned by platforms."

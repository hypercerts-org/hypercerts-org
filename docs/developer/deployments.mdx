...
export const DeploymentsTable = ({ deployments, color }) => {
  ...
  const isMainnet = (chainId) => ["10", "11155111", "42161"].includes(chainId);
...
The backend uses Railway to host the [API](./api.md) and [Indexer](./indexer.md) and supabase to host the data. 
